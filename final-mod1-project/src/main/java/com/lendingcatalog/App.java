...
public class  App {
    ...
    public static void main(String[] args) throws FileStorageException {
        try {
            ...
            System.out.println("There was an error with a file " + e.getMessage());
        }
    }
    ...
    private void initialize() throws FileStorageException {
        ...
        for (String line: lines){
            ...
            Member member = new Member(values[0], values[1]);
            List<CatalogItem> items = new ArrayList<>();
            List<String> itemLines = FileStorageService.readContentsOfFile(FILE_BASE_PATH + values[2]);

            for (String itemText: itemLines){
                items.add(itemFactory.parseItem(itemText));
            }
            catalog.put(member.toString(), items);
        }
    }

    private List<CatalogItem> loadItemsFromFile(String filename) throws FileStorageException {
        List<CatalogItem> items = new ArrayList<>();

        List<String> itemLines = FileStorageService.readContentsOfFile(FILE_BASE_PATH + filename);
